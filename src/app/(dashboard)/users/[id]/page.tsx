...
export default function UserDetailPage({ params }: { params: { id: string } }) {
  ...
  const fetchUser = async () => {
    try {
      setLoading(true);
      const response = await strapiClient.getDocument('users', params.id);
  ...
  const handleDelete = async () => {
    try {
      await strapiClient.deleteDocument('users', params.id);
  ...
  if (!user) {
    return (
      <div className="text-center py-12">
        <p className="text-slate-500">사용자를 찾을 수 없습니다.</p>
      </div>
    );
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">사용자명</p>
              <p className="text-base text-slate-900 dark:text-white mt-1">
                {user.username}
              </p>
            </div>
            ...
            <div>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">제공자</p>
              <p className="text-base text-slate-900 dark:text-white mt-1">
                {user.provider || 'local'}
              </p>
            </div>
